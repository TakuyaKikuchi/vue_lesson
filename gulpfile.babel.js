...
const config = {
  ...
  pubDir: './htdocs/',
  copyDevDir: {
    json: './_dev/_data/data.json',
    fonts: './_dev/**/assets/fonts/',
    images: './_dev/**/assets/images/'
  }
};

const targetBrowsers = ['ie >= 11', 'iOS >= 9', 'Android >= 5'];

gulp.task('pugs', () => {
  return gulp.src([`${config.devDir}**/*.pug`, `!${config.devDir}**/_*.pug`])
    .pipe(data(file => {
      return JSON.parse(fs.readFileSync('./_dev/_data/data.json'));
    }))
...
gulp.task('styles', () => {
  return gulp.src([`${config.devDir}**/*.scss`])
    .pipe(plumber({ errorHandler: notify.onError("Error: <%= error.message %>") }))
    .pipe(sass())
    .pipe(postcss([
      autoprefixer({
...
gulp.task('watch', ['browserSync'], () => {
  ...
  watch([ `${config.copyDevDir.json}`], () => gulp.start('Reload::html'));
